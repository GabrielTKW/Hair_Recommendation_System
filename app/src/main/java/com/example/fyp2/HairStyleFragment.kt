...
class HairStyleFragment : Fragment() {
    ...
    private lateinit var binding: FragmentHairStyleBinding
    ...
    override fun onCreateView(
        ...
        val fab: FloatingActionButton = binding.addHairstyleButton
        fab.setOnClickListener {
            nav.navigate(R.id.UploadFragment,bundleOf("userId" to id))
        }
        ...
        binding.recycleViewHair.adapter = adapter
        binding.recycleViewHair.addItemDecoration(DividerItemDecoration(context,DividerItemDecoration.VERTICAL))


        vm.getResult().observe(viewLifecycleOwner){ list->
            adapter.submitList(list)
            binding.txtCount.text = "${list.size} hairstyle(s)"
        }


        binding.recycleViewHair.apply{
    ...
    private fun replaceFragmentWithBundle(fragment: Fragment, bundle: Bundle) {
        fragment.arguments = bundle  // Set the bundle to the new fragment
        parentFragmentManager.beginTransaction().replace(R.id.frame_container, fragment).commit()
    }
