...
class userFavouriteFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val userFavorites = vmFavourite.getAllByUserId(id)

        vm.getAllFavourite(userFavorites).observe(viewLifecycleOwner) { list ->
            adapter.submitList(list)
            binding.txtCount2.text = "${list.size} hairstyle(s)"
        }

        binding.recyclerViewHair2.apply {
            layoutManager = LinearLayoutManager(requireContext())
            // Set the adapter to the RecyclerView
            adapter = this@userFavouriteFragment.adapter
        }
